...
| **Every session-bus peer is trusted with the whole history and with typing into focused windows.** A session bus authenticates nobody, so any process running as the user can call `List` → `Reveal` for the history in the clear, and `Paste` to have `clippod` type a chosen entry into whatever has keyboard focus. The escalation is not "a local process reads local files" — it is that clippod **re-exports two privileged Wayland capabilities** over a channel sandboxes routinely leave open *because* it is assumed not to carry them. A Flatpak app with `--socket=session-bus` is denied data-control and `zwp_virtual_keyboard_v1` by the proxied Wayland socket, and gets both back through clippo. | Three things, and none of them is a boundary. **(1)** `Paste` checks its caller: pid from `GetConnectionCredentials`, `/proc/<pid>/exe` against the installed clippo binaries, `/.flatpak-info` a refusal — `clippo-ipc::peer`. Pids are reusable and an allowlisted binary can be driven by whoever started it, so this narrows impersonation from *anything on the bus* to *anything that can be, or can drive, the real binary*: a speed bump. A uid check would be vacuous — every peer on a session bus is the same uid, which also rules out bus policy. **(2)** `allow_privileged_members = false` turns `Reveal` and `Paste` off entirely, enforced in the daemon; `List`, `Search` and `Copy` keep working. **(3)** This row. The genuinely correct answer is compositor-mediated access — a portal — which does not exist for clipboard managers yet. |
| **A peer can *be* the history.** Nothing stops another process taking `com.nilfactor.Clippo` while `clippod` is down, and it does not need to win a race at login: it queues for the name (no `DoNotQueue`), waits for any exit — a crash, an OOM kill, a `systemctl --user restart` — and the restarting daemon then finds the name held, exits, and is given up on by `StartLimitBurst`. What it gets is **display and input capture, not clipboard write**: every character typed into the picker's search field, because the applet sends the whole query to `Search` on each keystroke, and full control of every row, revealed value and thumbnail the user is shown. | **The name-request flags are not the lever they look like** — see `clippod::acquire_name`, which records why at length. `ReplaceExisting` cannot evict a squatter (it needs the *current* owner to have asked for `AllowReplacement`), `AllowReplacement` on clippod's own request would let any peer take the name from a live daemon in one call, and dropping `DoNotQueue` reintroduces the silent second instance it exists to prevent. There is no configuration-level defence. What there is: the frontends check the *owner* with the same `peer` helper — at connect time and again on every `NameOwnerChanged` that gives the name an owner — send nothing to one that fails, and **say so where the user is looking**. The picker draws the refusal and `clippo` exits non-zero with it; silent reconnection is what made this invisible. Same speed bump, same caveats, pointed the other way. |
